...
n_distinct(df.ex1.trials$workerid)
```
...
df.demos.ex1_rep <- read_csv("../../data/experiment_1_replication/prolific_demographics.csv") %>% 
  filter(Status=='AWAITING REVIEW')

#bulk import data
file_list <- list.files(path = "../../data/experiment_1_replication/raw_data", pattern = "\\.csv$", full.names = TRUE)
...
for (file in file_list) {
  df <- read_csv(file, col_types = cols(.default = col_character())) %>% 
    mutate(workerid = substr(file,46,55)) #add particiant's random id from filename 
  df_list[[length(df_list) + 1]] <- df
}

# Combine all dataframes into a single dataframe
df.import.ex1_rep <- bind_rows(df_list)

#create individual datasets
df.ex1_rep.trials <- df.import.ex1_rep %>% 
  filter(prolific_id %in% df.demos.ex1_rep$`Participant id`) %>% #REMOVE PARTICIPANTS WHO DIDN'T COMPLETE PROPERLY
  filter(trial=='prediction_trial') %>% 
  select(-prolific_id, -prolific_study_id, -prolific_session_id, -feedback, -age, -ethnicity, -gender, -race, -overall_accuracy, -bonus, -correct_response, -stimulus_dict, -view_history, -success, -timeout, -failed_images, -failed_audio, -failed_video)

df.ex1_rep.participants <- df.import.ex1_rep %>% 
  filter(trial=='exit_survey')

write.csv(df.ex1_rep.trials, "../../data/experiment_1_replication/ex1_rep_trials.csv")
write.csv(df.ex1_rep.participants, "../../data/experiment_1_replication/ex1_rep_participants.csv")
```
...
##Calculate Bonus info
```{r, eval=FALSE}
#select bonuses with prolific ids
df.ex1_rep.rewards <- df.ex1_rep.participants %>% 
  select(prolific_id, bonus) %>% 
  mutate(bonus = round(as.numeric(bonus), 2))

#write out bonus csv
write.table(df.ex1_rep.rewards,
            "../../data/experiment_1_replication/experiment_1_rep-bonuses.csv",
            sep = ",", 
            col.names = FALSE,
            row.names = FALSE,
            quote = FALSE)
...
```{r}
#get demos
df.ex1_rep.demos <- read_csv("../../data/experiment_1_replication/ex1_rep_participants.csv") %>% 
...
aov.6 <- anova(glm.12, glm.13)

aov.6
```
###Pairwise Followup
```{r}
emmeans(glm.12, pairwise ~ ego_can_infer)
```
...
ggsave(filename = "../../figures/plots/alter_knowledge_replication.pdf",
       width = 5,
       height = 4)
...
combined_select_ball_means <- combined_select_ball %>% 
  ...
  summarize(means = mean(proportional_response), sd = sd(proportional_response))
...
##Combined Results: Arrange Method
```{r}
...
fig1_controlchar <- fig1_controlchar + 
...
ggsave(filename = "../../figures/plots/combined_fig.pdf", combined_fig,
       width = 10,
       height = 8)
```
